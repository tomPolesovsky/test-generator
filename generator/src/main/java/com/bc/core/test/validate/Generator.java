...
public class Generator extends AbstractGenerator {
	...
	public static boolean isValidateMethod(MetaClass metaCls, String method) {
		if (method.startsWith("validate")) {
			String attr = method.replaceFirst("^validate", "").toLowerCase();
			List<Parameter> attrs = metaCls.getAttributes();

			for (Parameter at : attrs) {
				if (at.getName().equals(attr) && at.getDefaultVals().length != 0) {
					return true;
				}
			}
		}
	...
	public void generate() {
		...
		com.bc.core.test.validate.ScenarioFactory factory = new com.bc.core.test.validate.ScenarioFactory();
